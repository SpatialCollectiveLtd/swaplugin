    ...
    private void applyCleanSlateFilter(OsmDataLayer layer) {
        GuiHelper.runInEDT(() -> {
            try {
                ...
                var filterModel = MainApplication.getMap().filterDialog.getFilterModel();
                
                // Check if our filter already exists
                boolean filterExists = false;
                for (int i = 0; i < filterModel.getRowCount(); i++) {
                    var filter = filterModel.getValue(i);
                ...
                if (!filterExists) {
                    var newFilter = new org.openstreetmap.josm.data.osm.Filter();
